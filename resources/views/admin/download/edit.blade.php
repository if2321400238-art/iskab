...
<div class="max-w-3xl mx-auto">
    <div class="flex items-center justify-between mb-6">
        <h1 class="text-2xl font-bold text-gray-800">Edit File Download</h1>
    ...
    <form action="{{ route('admin.download.update', $download) }}" method="POST" enctype="multipart/form-data" class="bg-white shadow rounded p-6 space-y-4">
        @csrf
        @method('PUT')
        <div>
            <label class="block text-sm font-medium text-gray-700 mb-1">Nama File</label>
            <input type="text" name="nama_file" value="{{ old('nama_file', $download->nama_file) }}" class="w-full border-gray-300 rounded" required>
        </div>
        <div>
        ...
        <div>
            ...
            <textarea name="deskripsi" rows="3" class="w-full border-gray-300 rounded">{{ old('deskripsi', $download->deskripsi) }}</textarea>
        </div>
        <div>
            <label class="block text-sm font-medium text-gray-700 mb-1">File (opsional ganti)</label>
            <input type="file" name="file_path" class="w-full border-gray-300 rounded">
            <p class="text-xs text-gray-500 mt-1">File saat ini: {{ $download->file_path }}</p>
        </div>
