...
@section('page_title', 'Tambah Korwil')

@section('content')
<div class="max-w-2xl mx-auto">
    <div class="bg-white rounded-lg shadow-md p-8">
        <h2 class="text-2xl font-bold mb-6">Tambah Korwil Baru</h2>

        <form action="{{ route('admin.korwil.store') }}" method="POST" class="space-y-6">
            @csrf
            ...
            <div>
                ...
                <input type="text" id="nomor_sk" name="nomor_sk" value="{{ old('nomor_sk') }}"
                    ...
                    placeholder="contoh: SK/BPH-PB/001/2025">
                @error('nomor_sk')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                <textarea id="description" name="description" rows="4"
                    ...
                    placeholder="Deskripsi tentang Korwil ini...">{{ old('description') }}</textarea>
                @error('description')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
